...
% --------------------------------------------------
\subsubsection{Reactivity and Temperature range}
...
Using the previous expressions for the plasma volume and the reactivity, the fusion power reads (in MW):

\begin{equation}
\boxed{
  \hat P_{fus} = C_{fus} C_V \varepsilon^2 R^3 \hat n^2 \hat T^2
  }
\label{eq:DT_fusion_power}
\end{equation}

\noindent
with $C_{fus} \approx 17.59 \times e\times 1.18\, 10^{-24} \times 10^{2\times19}\times 1/4$. Here that $\hat T$ is expressed in keV, and $\hat n$ in $10^{19} \, \si{m^{-3}}$. 
